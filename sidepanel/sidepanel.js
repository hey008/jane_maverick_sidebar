...
async function renderManagePanel() {
  ...
  // Default sites — collapsible section header + sidebar visibility toggle
  const sectionHeader = document.createElement('div');
  sectionHeader.className = 'manage-section-header';

  const chevronSvg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
  chevronSvg.setAttribute('viewBox', '0 0 24 24');
  chevronSvg.classList.add('manage-section-chevron');
  if (_defaultSitesExpanded) chevronSvg.classList.add('expanded');
  const chevronPath = document.createElementNS('http://www.w3.org/2000/svg', 'path');
  chevronPath.setAttribute('d', 'M8.59 16.59L13.17 12 8.59 7.41 10 6l6 6-6 6z');
  chevronSvg.appendChild(chevronPath);

  const sectionLabel = document.createElement('span');
  sectionLabel.className = 'manage-section-label';
  sectionLabel.textContent = 'Default Sites';

  const spacer = document.createElement('span');
  spacer.style.flex = '1';

  const EYE_ON  = '<path d="M12 4.5C7 4.5 2.73 7.61 1 12c1.73 4.39 6 7.5 11 7.5s9.27-3.11 11-7.5c-1.73-4.39-6-7.5-11-7.5zM12 17c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-8c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3z"/>';
  const EYE_OFF = '<path d="M12 7c2.76 0 5 2.24 5 5 0 .65-.13 1.26-.36 1.83l2.92 2.92c1.51-1.26 2.7-2.89 3.43-4.75-1.73-4.39-6-7.5-11-7.5-1.4 0-2.74.25-3.98.7l2.16 2.16C10.74 7.13 11.35 7 12 7zM2 4.27l2.28 2.28.46.46C3.08 8.3 1.78 10.02 1 12c1.73 4.39 6 7.5 11 7.5 1.55 0 3.03-.3 4.38-.84l.42.42L19.73 22 21 20.73 3.27 3 2 4.27zM7.53 9.8l1.55 1.55c-.05.21-.08.43-.08.65 0 1.66 1.34 3 3 3 .22 0 .44-.03.65-.08l1.55 1.55c-.67.33-1.41.53-2.2.53-2.76 0-5-2.24-5-5 0-.79.2-1.53.53-2.2zm4.31-.78l3.15 3.15.02-.16c0-1.66-1.34-3-3-3l-.17.01z"/>';
  const visBtn = document.createElement('button');
  visBtn.className = `manage-section-vis-btn${_showDefaultInBar ? '' : ' hidden'}`;
  visBtn.title = _showDefaultInBar ? 'Hide from sidebar' : 'Show in sidebar';
  visBtn.innerHTML = `<svg viewBox="0 0 24 24">${_showDefaultInBar ? EYE_ON : EYE_OFF}</svg>`;
  visBtn.addEventListener('click', async (e) => {
    e.stopPropagation();
    _showDefaultInBar = !_showDefaultInBar;
    await chrome.storage.local.set({ showDefaultInBar: _showDefaultInBar });
    renderSitesBar();
    renderManagePanel();
  });

  sectionHeader.append(chevronSvg, sectionLabel, spacer, visBtn);
